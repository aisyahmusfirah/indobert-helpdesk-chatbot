...
def load_bert_vocab() -> Dict[str, int]:
    """
    Load Indonesian BERT vocabulary, downloading and saving it to config.bert_chinese_vocab if needed.
    """
    import os
    from transformers import BertTokenizer
    
    vocab_path = Config.bert_chinese_vocab
    # Download/save if it does not exist or has the wrong size
    if not os.path.exists(vocab_path) or os.path.getsize(vocab_path) < 200000:
        print("Downloading 'indobenchmark/indobert-base-p2' vocabulary...")
        tokenizer = BertTokenizer.from_pretrained('indobenchmark/indobert-base-p2')
        os.makedirs(os.path.dirname(vocab_path), exist_ok=True)
        saved = tokenizer.save_vocabulary(os.path.dirname(vocab_path))
        # save_vocabulary may return a path or a tuple; normalize
        if isinstance(saved, (list, tuple)):
            saved = saved[0]
        # If the saved path isn't the exact expected path, move/rename it
        try:
            if saved and os.path.abspath(saved) != os.path.abspath(vocab_path):
                os.replace(saved, vocab_path)
        except Exception:
...
class Tokenizer(BasicTokenizer):
    ...
    def __init__(self, token_dict):
        ...
        from transformers import BertTokenizer
        self.hf_tokenizer = BertTokenizer.from_pretrained('indobenchmark/indobert-base-p2')
